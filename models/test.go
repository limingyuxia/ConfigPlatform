...
var TestWhere = struct {
	...
	Content:    whereHelperstring{field: "`test`.`content`"},
	CreateTime: whereHelpertime_Time{field: "`test`.`create_time`"},
	UpdateTime: whereHelpertime_Time{field: "`test`.`update_time`"},
}
...
var (
	testAllColumns            = []string{"id", "content", "create_time", "update_time"}
	testColumnsWithoutDefault = []string{"content"}
	testColumnsWithDefault    = []string{"id", "create_time", "update_time"}
...
func FindTest(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Test, error) {
	...
	query := fmt.Sprintf(
		"select %s from `test` where `id`=?", sel,
	)
...
func TestExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `test` where `id`=? limit 1)"
